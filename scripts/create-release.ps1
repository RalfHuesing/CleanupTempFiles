...
.DESCRIPTION
    1. Liest die Version aus src/CleanupTempFiles/CleanupTempFiles.csproj
...
$ProjectFile = 'src/CleanupTempFiles/CleanupTempFiles.csproj'
$VersionPattern = '(?<=<Version>)\d+\.\d+\.\d+(?=</Version>)'

function Get-RepoRoot {
    $root = git rev-parse --show-toplevel 2>$null
    if (-not $root) {
        throw 'Kein Git-Repository gefunden. Bitte im CleanupTempFiles-Repo ausfuehren.'
    }
...
function Wait-ReleaseWorkflow {
    ...
    if (-not (Test-GhAuthenticated)) {
        ...
        Write-Host '       https://github.com/RalfHuesing/CleanupTempFiles/actions/workflows/release.yml'
        return
    }
...
try {
    ...
    Write-Host ''
    Write-Host "CleanupTempFiles Release" -ForegroundColor White
